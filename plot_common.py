...
def _pilim_if_path(im):
    if type(im) == type(str()):
        _, f_extension = os.path.splitext(im)
        f_extension = f_extension.lower()
        if '.tif' in f_extension:
            with rasterio.open(im, 'r') as f:
                return f.read().shape[1:]
        else:
            return PIL.Image.open(glob(im + '*')[0]).size
    return im.size
...
def add_layout_images_to_fig(fig, images, update_ranges=True):
    ...
    for im in images:
        ...
        width, height = _pilim_if_path(im)
        # Add images
        #TODO
    ...
    if update_ranges:
        width, height = [
            max([_pilim_if_path(im)[i] for im in images]) for i in range(2)
        ]
