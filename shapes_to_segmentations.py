...
def label_to_colors(
    img, colormap=px.colors.qualitative.Light24, alpha_3=255, alpha_1=128, color_class_offset=0
):
    """
    ...
    # Get only boundaries
    alpha_values = np.zeros(img.shape[:2] + (1,), dtype="uint8")
    
    s = ndimage.generate_binary_structure(2,2)
    for i in np.unique(img):

        binary = img==i
        boundary = ndimage.binary_erosion(binary, structure=s, iterations=3) ^ binary
        alpha_values[boundary]=1
        
    return np.concatenate((cimg, alpha_3 * alpha_values), axis=2), np.concatenate(
            (cimg, alpha_1 * np.ones(img.shape[:2] + (1,), dtype="uint8")), axis=2)
...
def compute_segmentations(
    ...
    clf = RandomForestClassifier(
        ...
        n_estimators=95, n_jobs=-1, max_depth=8, max_samples=0.3, class_weight='balanced_subsample'
    )
    seg, clf = fit_segmenter(mask, features, clf)
    t2 = time()
    print(t2 - t1)
    
    boundary_seg, color_seg = label_to_colors(seg, **label_to_colors_args)
    # color_seg is a 3d tensor representing a colored image whereas seg is a
    # matrix whose entries represent the classes
    return (boundary_seg, color_seg, seg, clf)
