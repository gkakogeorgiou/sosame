...
def fit_segmenter(labels, features, clf):
    ...
    features = np.moveaxis(features, [0,1,2], [2, 0, 1])
    W,H,F = features.shape
    features = features.reshape((-1,F))
    
    y_pred_all = clf.predict(features)
    y_pred_all = y_pred_all.reshape((W,H))
    #io.imsave('y_pred_all_before.png', y_pred_all)
    # # New logic
    drop=0.0055
    s = ndimage.generate_binary_structure(2,2)
    
    y_pred_all = median(y_pred_all, s)
    
    for i in np.unique(y_pred_all):

        binary = y_pred_all==i
        components, _ = ndimage.label(binary, structure=s)

        threshold = W*H*drop
        for j in np.unique(components)[1:]:
            temp_comp = components == j

            if temp_comp.sum()< threshold:
                boundary = ndimage.binary_dilation(temp_comp, structure=s) ^ temp_comp
                y_pred_all[temp_comp] = Counter(y_pred_all[boundary]).most_common(1)[0][0]

    ################################
    #io.imsave('y_pred_all_after.png', y_pred_all)
    output = np.copy(labels)
    output[labels == 0] = y_pred_all[labels == 0]
    #io.imsave('y_pred_all_final.png', y_pred_all)
    return output, clf
